...
def write_blockmesh(case,threed,blunt_te,n_blades,rad,chords,camber,stagger,t_c,stack,
	farfield=5.0,grid_r=60,grid_c=40, grid_s=8,grid_le=50,grid_w=50,
	exp_r=100,exp_lete=1.0,exp_wall=10.0,foam_version="2.0"):
	c_0 = chords[0]
	passage_angle = 2*pi/n_blades
	# profile file names for axial flow pump
	#profile = {rad[0]:"rotor_hub.csv",rad[1]:"rotor_mid.csv",rad[2]:"rotor_tip.csv"}
	...
	def generate_geom(i):
		...
		[p,s] = read_ps_profile(profile[rad[i]],chords[i],-stagger[i])
		#leading edge gradient
		legrad = ((s[1][2]+p[1][2])/2.0-s[1][1])/((s[0][2]+p[0][2])/2.0-s[0][1])
		#trailing edge gradient
		tegrad = (s[1][-1]-(s[1][-3]+p[1][-3])/2.0)/(s[0][-1]-(s[0][-3]+p[0][-3])/2.0)
		...
		geom = [(s[0][1],s[1][1]),			#0
			...
			(p[0][1],p[1][1]),			#5
			(s[0][0]-farfield*chords[i],yoff+s[1][0]-farfield*c_0*legrad),	#6
			(s[0][len(s[0])/2],yoff*0.8),					#7
			(s[0][-1],yoff+s[1][-1]),					#8
			(s[0][-1]+farfield*chords[i],yoff+s[1][-1]+farfield*c_0*tegrad),	#9
			(s[0][-1]+farfield*chords[i],yoff*0.1+s[1][-1]+farfield*c_0*tegrad),	#10
			(s[0][-1]+farfield*chords[i],-yoff*0.1+s[1][-1]+farfield*c_0*tegrad),	#11
			(s[0][-1]+farfield*chords[i],-yoff+s[1][-1]+farfield*c_0*tegrad),	#12
			(s[0][-1],-yoff+s[1][-1]),					#13
			(s[0][len(s[0])/2],-yoff*1.2),					#14
			(s[0][0]-farfield*chords[i],-yoff+s[1][0]-farfield*c_0*legrad)]	#15
		return geom
	...
	# start of actual write_blockmesh function
...
""")
	...
	for i in range(len(chords)):
		...
		if(not blunt_te):
			if(threed):
				f.write("\tarc "+str(2+i*16)+" "+str(3+i*16)+" ("+str(xoff+s[0][-1])+" "+
					str(rad[i]*sin(s[1][-1]/rad[i]))+" "+str(rad[i]*cos(s[1][-1]/rad[i]))+")\n")
			else:
				f.write("\tarc "+str(2+i*16)+" "+str(3+i*16)+" ("+str(xoff+s[0][-1])+" "+
					str(s[1][-1])+" "+str(i*chords[i]/10.0)+")\n")
...
blunt_te = False # set this to True to make trailing edge blunt instead of radius
...
omega = 997 #rpm
rad_name = ["r_hub","r_mid","r_tip"]
rad = [0.130,0.1762,0.228]
chords = [0.216,0.2265,0.2401]
stagger	= [52.0,66.0,71.75]
camber = [0,0,0] # dummy variable
farfield = 0.5 #number of chord lengths to farfield boundary
profile = {rad[0]:"rotor_hub.csv",rad[1]:"rotor_mid.csv",rad[2]:"rotor_tip.csv"}

# inlet velocity
blade_speed = [omega*2*pi/60*r for r in rad]
flowrate = [x/1000.0/(pi*(rad[-1]**2-rad[0]**2)) for x in range(600,1000,100)]
# create blockmesh file for wing
for i in range(len(chords)):
	for cz in flowrate:
		print(cz)
		chord = chords[i]
		gamma = -stagger[i] # remember the minus-sign
		case = rad_name[i]+"_cz"+str(int(round(cz*1000.0*pi*(rad[-1]**2-rad[0]**2),0))) #case directory name
		# copy the case from the template
		CloneCase(args=["2d_case_template",case])
		# set the inlet velocity BC
		velFile = ParsedParameterFile(os.path.join(case,"0","U"))
		velFile["boundaryField"]["inlet"]["value"].setUniform(Vector(cz,blade_speed[i],0))
		velFile.writeFile()
		# create blockmesh file for wing
		write_blockmesh(case,threed,blunt_te,n_blades,[rad[i],],[chords[i],],[camber[i],],[stagger[i],],th,stack,farfield=1.0,grid_le=20,grid_w = 240)
		# run blockMesh on the case
		Runner(args=["--clear","blockMesh", "-case",case])
		# run createPatch on the case
		Runner(args=["--clear","createPatch","-case",case,"-overwrite"])
		...
		# run the simulation
		Runner(args=["--progress","simpleFoam","-case",case])
